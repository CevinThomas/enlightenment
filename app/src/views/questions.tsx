...
const Questions = (props) => {
  ...
  useEffect(() => {
    if (props.route && props.route.params.questions !== undefined) {
      ...
      Alert.alert(
        "Something went wrong with this category, please try again or another."
      );
    }
  ...
  async function load(idToUse: number) {
    ...
    questionsBeingUsedRef.current.forEach((question) =>
  ...
  function isNextQuestionAnswered(): boolean | void {
    ...
    if (
      currentQuestion.answered === IsAnswered.yes &&
      questions.isLastQuestionComing === 1
    ) {
      return viewNextQuestion(questions.nextQuestion);
    }

    if (
      questions.nextQuestion.answered === IsAnswered.no &&
      secondNextQuestion.answered === IsAnswered.no
    ) {
  ...
  async function updateQuestions(
    ...
    for (let i = 0; i < allQuestions.length; i++) {
      if (
        currentQuestion.name === allQuestions[i].name &&
        i === questionsLength &&
        removedQuestionsNames.includes(currentQuestion.name)
      ) {
        return Alert.alert(
          "You are on the last question, please answer the question, and remove it in the next round."
        );
      }
    }
    ...
    try {
      ...
      await AsyncStorage.setItem(
        ...
        JSON.stringify(questionsToBeSaved)
      );
